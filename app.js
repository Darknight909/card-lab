...
async function restoreDraft(){try{const d=await draftGet();if(!d)return;if(d.frontData?.dataUrl){frontData=d.frontData;$('frontPreview').src=frontData.dataUrl;$('frontPreview').style.display='block';$('frontQuality').innerHTML=qualityText(frontData.quality)}if(d.backData?.dataUrl){backData=d.backData;$('backPreview').src=backData.dataUrl;$('backPreview').style.display='block';$('backQuality').innerHTML=qualityText(backData.quality)}if(frontData||backData)toast('Draft photos restored')}catch(e){console.warn('Could not restore draft photos',e)}}
...
async function applyBackendAnalysis(result){
  ...
  if(autoCenteringReady){applyPairToBorders('front','lr',c.front?.lr);applyPairToBorders('front','tb',c.front?.tb);applyPairToBorders('back','lr',c.back?.lr);applyPairToBorders('back','tb',c.back?.tb)}
