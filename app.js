...
let db, frontData=null, backData=null, lastEstimate=null, deferredPrompt=null, ocrRaw={front:'',back:''}, ocrSuggestions={}, backendAnalysis=null, ebayData=null, marketData=null, currentCardId=null, analysisInFlight=false, autoIdentityReady=false, autoCenteringReady=false, autoConditionReady=false;
...
async function persistDraft(){try{await draftPut({frontData,backData,updatedAt:new Date().toISOString()})}catch(e){console.warn('Could not save draft photos',e)}}
async function restoreDraft(){try{const d=await draftGet();if(!d)return;if(d.frontData?.dataUrl){frontData=d.frontData;$('frontPreview').src=frontData.dataUrl;$('frontPreview').style.display='block';$('frontQuality').innerHTML=qualityText(frontData.quality);if($('frontSavedStatus'))$('frontSavedStatus').textContent='Photo saved locally'}if(d.backData?.dataUrl){backData=d.backData;$('backPreview').src=backData.dataUrl;$('backPreview').style.display='block';$('backQuality').innerHTML=qualityText(backData.quality);if($('backSavedStatus'))$('backSavedStatus').textContent='Photo saved locally'}if(frontData||backData)toast('Draft photos restored')}catch(e){console.warn('Could not restore draft photos',e)}}
...
async function handlePhoto(input,preview,quality,side){
  const f=input.files?.[0];if(!f)return;try{const o=await compressImage(f);$(preview).src=o.dataUrl;$(preview).style.display='block';$(quality).innerHTML=qualityText(o.quality);if(side==='front'){frontData=o;if($('frontSavedStatus'))$('frontSavedStatus').textContent='Photo saved locally'}else{backData=o;if($('backSavedStatus'))$('backSavedStatus').textContent='Photo saved locally'}await persistDraft();ocrRaw[side]='';ocrSuggestions={};backendAnalysis=null;ebayData=null;marketData=null;currentCardId=null;autoIdentityReady=autoCenteringReady=autoConditionReady=false;$('identifyResults').classList.add('hidden');lastEstimate=null;updateCentering();renderConditionSummary();toast(`${side} photo ready · saved locally`);if(frontData&&backData&&!analysisInFlight)setTimeout(()=>identifyFromPhotos(),250)}catch(e){toast('Could not process photo')}
}
...
async function applyBackendAnalysis(result){
  ...
  autoIdentityReady=Boolean(i.year && (i.set||i.brand) && i.subject && i.cardNo);
  const validPair=p=>Array.isArray(p)&&p.length===2&&p.every(v=>Number.isFinite(Number(v))&&Number(v)>0);
  autoCenteringReady=validPair(c.front?.lr)&&validPair(c.front?.tb)&&validPair(c.back?.lr)&&validPair(c.back?.tb);
  if(!autoCenteringReady && frontData && backData){try{await autoBorders('front',true);await autoBorders('back',true);autoCenteringReady=true;}catch(e){console.warn('Local centering fallback failed',e)}}
  autoConditionReady=[c.corners,c.edges,c.surface,c.focus].every(v=>Number.isFinite(Number(v))&&Number(v)>=1&&Number(v)<=10) && Number(a.condition_confidence||0)>=45;
  ...
  if(validPair(c.front?.lr)&&validPair(c.front?.tb)&&validPair(c.back?.lr)&&validPair(c.back?.tb)){applyPairToBorders('front','lr',c.front?.lr);applyPairToBorders('front','tb',c.front?.tb);applyPairToBorders('back','lr',c.back?.lr);applyPairToBorders('back','tb',c.back?.tb)}
  ...
  if(autoIdentityReady){await saveCardAutomatic();if(backendAnalysis?.needs_review&&$('saveStatus'))$('saveStatus').textContent+= ' · identity flagged for review';}
  else if($('saveStatus'))$('saveStatus').textContent='Not saved yet: automatic identification is incomplete. Re-analyze after the backend finishes a reliable match.';
}
async function resizeDataUrlForAnalysis(dataUrl,max=1400,q=.78){
  try{
    const img=new Image();img.src=dataUrl;await img.decode();
    const scale=Math.min(1,max/Math.max(img.naturalWidth||img.width,img.naturalHeight||img.height));
    if(scale>=.98)return dataUrl;
  ...
  for(let attempt=1;attempt<=3;attempt++){
    try{
      ...
      if(!r.ok||!j.ok){
        ...
        if(r.status>=500&&attempt<3){lastErr=err;await new Promise(x=>setTimeout(x,900*attempt));continue}
        throw err;
      }
      return j;
    }catch(e){
      lastErr=e;
      const networkish=e instanceof TypeError || /load failed|network|fetch/i.test(String(e?.message||e));
      if(attempt<3&&networkish){await new Promise(x=>setTimeout(x,900*attempt));continue}
      throw e;
    }
  }
...
async function identifyFromPhotos(){
  ...
  try{
    ...
    const [frontForAnalysis,backForAnalysis]=await Promise.all([
      ...
      resizeDataUrlForAnalysis(backData.dataUrl)
    ]);
...
function updateCentering(){for(const side of ['front','back']){if(!autoCenteringReady){$(side+'CenteringReadout').textContent='Waiting for automatic analysis';continue}const c=centeringSide(side);$(side+'CenteringReadout').textContent=`L/R ${c.lr[0].toFixed(1)}/${c.lr[1].toFixed(1)} · T/B ${c.tb[0].toFixed(1)}/${c.tb[1].toFixed(1)}`}}
...
function renderEstimate(){if(!autoConditionReady||!autoCenteringReady){lastEstimate=null;$('gradeResults').classList.remove('empty');$('gradeResults').innerHTML='<div class="warn"><strong>Automatic grade not available yet.</strong> Reliable condition and centering data were not returned, so Card Lab will not invent a grade.</div>';return}lastEstimate=companyGrades();const e=lastEstimate;const q=e.confidence>=80?'good':e.confidence>=60?'warn':'bad';$('gradeResults').classList.remove('empty');$('gradeResults').innerHTML=`<div class="grade-grid">
...
async function exportBackup(){const data=await dbAll();downloadJson({version:2,type:'collection',exportedAt:new Date().toISOString(),cards:data},`card-lab-collection-${new Date().toISOString().slice(0,10)}.json`)}
async function exportRecovery(){if(!confirm('Full recovery contains your private Cloudflare API key and draft card photos. Export and store it securely?'))return;const data=await dbAll();const draft=await draftGet();downloadJson({version:2,type:'full-recovery',exportedAt:new Date().toISOString(),cards:data,draft,settings:{backendUrl:localStorage.getItem('cardlab.backendUrl')||'',backendKey:localStorage.getItem('cardlab.backendKey')||''}},`card-lab-full-recovery-${new Date().toISOString().slice(0,10)}.json`);toast('Full recovery exported')}
async function importBackup(file){try{const j=JSON.parse(await file.text());if(!Array.isArray(j.cards))throw 0;for(const c of j.cards){delete c.id;await dbAdd(c)}if(j.settings){localStorage.setItem('cardlab.backendUrl',j.settings.backendUrl||'');localStorage.setItem('cardlab.backendKey',j.settings.backendKey||'');loadBackendSettings()}if(j.draft){await draftPut({frontData:j.draft.frontData||null,backData:j.draft.backData||null,updatedAt:j.draft.updatedAt||new Date().toISOString()});await restoreDraft()}toast(`Imported ${j.cards.length} cards${j.settings?' + settings':''}`);renderCollection()}catch{toast('Invalid backup file')}}
...
async function registerUpdater(){
  ...
  try{
    ...
    await reg.update();activateWaiting();
  }catch(e){console.warn('Updater registration failed',e)}
}
async function checkForUpdate(manual=false){
  const now=Date.now();if(!manual&&now-lastUpdateCheck<120000)return;lastUpdateCheck=now;
  const status=$('updateStatus');if(manual&&status)status.textContent='Checking…';
  try{
    const r=await fetch(`./version.json?t=${now}`,{cache:'no-store'});if(!r.ok)throw new Error(`HTTP ${r.status}`);const j=await r.json();
    if(j.version&&j.version!==APP_VERSION){if(status)status.textContent=`Update v${j.version} found · installing`;toast(`Card Lab v${j.version} update found`);await registerUpdater();setTimeout(()=>location.reload(),900)}
    else {if(status)status.textContent=`v${APP_VERSION} is current`;if(manual)toast(`Card Lab v${APP_VERSION} is current`)}
  }catch(e){if(status)status.textContent='Update check unavailable';if(manual)toast('Could not check for update')}
}
